...
(defn evaluate [exp env]
  (cond
    ...
    (= (get exp 0) 'if) (if (evaluate (exp 1) env) (evaluate (exp 2) env) (evaluate (get exp 3) env))
    (= (get exp 0) 'quote) (exp 1)
    (= (get exp 0) 'q-quote) (evaluate (handle-quasi (exp 1)) env)
...
(defn reader [s eval?]
  (cond
    ...
    (and eval? (re-find #"^\(\s*(?:lambda|quote|q-quote|define-macro|if|begin)\s+" s)) (let [[res rmn] (reader s false)] [(evaluate res :g) (trim rmn)])
    (re-find #"^\(" s) (loop [rst (trim (replace-first s #"^\(" "")), exp []]
                         (cond
                           ...
                           :else (when-let [[res rmn] (reader rst eval?)] (recur (trim rmn) (conj exp res)))))
...
;(define-macro when (lambda (cnd . then) (quasi-quote (if (unquote cnd) (begin (unquote-splice then)) nil))))
